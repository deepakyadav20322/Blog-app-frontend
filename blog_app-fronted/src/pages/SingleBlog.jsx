...
const SingleBlog = () => {
  ...
  let likeStateToggle =
    realTimeallLikes.length > 0 ? realTimeallLikes : allLikes;
  ...
  const UserFollow = async (id) => {
    try {
      ...
      if (res.status === 200) {
        ...
        setApiCalled(false);
        toast.success(
          `You follow to  ${blog?.author.fname}  ${blog?.author.lname}`,
          {
            duration: 2500,
            className: "toasti",
  ...
  const UserUnFollow = async (id) => {
    try {
      ...
      if (res.status === 200) {
        ...
        setApiCalled(false);
        toast.success(
          `You Unfollow to ${blog?.author.fname}  ${blog?.author.lname}`,
          {
            duration: 2500,
            className: "toasti",
            position: "bottom-right",
  ...
  useEffect(() => {
    if (!apiCalled) {
      var fetchSingleBlogPost = async () => {
        try {
          ...
          const res = await axios.get(
            `${baseURL}/post/getSinglePost/${blogId}`
          );
          if (res.status === 200) {
          ...
          if (
            ...
            error.response.data.error == "Invalid post ID"
          ) {
      ...
      const getAllPostsUserRecomendation = async () => {
        try {
          const res = await axios.get(
            `${baseURL}/post/getAllPostsOfUserRecomendation/${blogId}`
          );
          if (res.status === 200) {
  ...
  const handleCommentOnSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${baseURL}/post/addCommentToPost/${blogId}`,
        commentData,
        {
          headers: {
      ...
      if (res.status === 200) {
        ...
        newComment.author = {
          ...
          profileImg: blogUser.profileImg,
        };

        setCommentData({ commentText: "" });
        console.log(newComment);
        setAllPostedComment((prevComments) =>
      ...
      const removePostFromSaved = async () => {
        try {
          ...
          const response = await axios.delete(
            `${baseURL}/post/unSavePost/${postId}`,
            {
              headers: {
                ContentType: "application/json",
                Authorization: `Bearer ${token}`,
          ...
          if (response.status === 200) {
            ...
            const updatedSavedPosts = blogUser?.user.savedPost.filter(
              (savedPost) => savedPost.post !== blogId
            );
  ...
  useEffect(() => {
    const savedPosts = JSON.parse(localStorage.getItem("blogUser"))?.user
      ?.savedPost;
    ...
    <div className="flex justify-center items-center min-h-[100vh] md:min-h-[40vh]">
      <div className="border-2 border-gray-300 rounded flex flex-row justify-center items-center p-10">
        <div className="text-4xl font-bold">
          No post available for this postId{" "}
        </div>
      </div>
    </div>;
  }
    ...
    <>
      {blog && (
        <div>
            ...
            <div className="flex justify-center pb-4 ">
              <div className="max-w-[730px] px-3 sm:px-6  w-full bg-white">
                <div className="block w-full">
                  <h1 className="w-full tracking-tighter-[-0.011em] sm:tracking-tighter-[-0.014em] sm:leading-[2.8rem] leading-9 mb-8 sm:mb-6 sm:text-[40px] text-3xl font-bold text-[#242424] sm:mt-12 mt-8">
                    {blog.title}
                  </h1>
                  <div className="">
                    <div className="flex flex-col justify-between">
                      <div className="block w-full">
                        <div className="flex ">
                          <Link to={`/writer/${blog?.author._id}`} className="">
                            <img
                              className="h-10 w-12 object-cover rounded-[50%] mt-1"
                              src={
                                blog?.author?.profileImg
                                  ? `${baseURL}/UserImages/${blog?.author.profileImg}`
                                  : userLogo
                              }
                            />
                          </Link>
                          ...
                          <div
                            className={`${
                              userId === blog.author._id ? "flex" : "hidden"
                            } rounded  py-2 sm:py-0 sm:flex-row sm:justify-center flex-col items-center justify-center sm:items-center bg-[#FEF5E6] h-12 sm:h-9 text-sm mr-2`}
                          >
                            <div className="">
                              <Link
                                className="mx-2 my-1 ms:my-0 hover:border-b-2 border-blue-500 hover:text-blue-500 transition-all duration-150"
                                to={`/updatePost/${blog._id}`}
                              >
                                Edit
                              </Link>{" "}
                            </div>
                            <div className="mx-2 hover:border-b-2 border-blue-500 hover:text-blue-500 transition-all duration-150">
                              Manage
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-row justify-between mt-7 border-t-[0.2px] border-b-[0.2px] border-gray-200 py-2">
                        <div className="flex flex-row gap-4">
                          ...
                          <span className="hover:text-black">
                            {!token && (
                              <AiOutlineHeart
                                size={25}
                                onClick={() => setLikeAllowedPopUp(true)}
                            ...
                            {token && blog.author._id === userId && (
                              <AiOutlineHeart
                                size={25}
                                className="inline cursor-not-allowed text-gray-300"
                              />
                            )}
                            {likeAllowedPopUp ? (
                              <FirstDoLoginPopUp
                                setAllowedPopUp={setLikeAllowedPopUp}
                              />
                            ) : (
                              ""
                            )}
                            ...
                            {token &&
                            likeStateToggle?.length >= 0 &&
                            likeStateToggle?.includes(userId) &&
                          ...
                          <span className="relative">
                            ...
                            >
                              <FaRegCommentDots
                                size={22}
                                className="hover:text-blue-500 inline cursor-pointer mr-1"
                              />
                            ...
                            <div
                              ...
                              } absolute w-28 leading-4 h-10 pt-[2px] bg-gray-900 opacity-90 text-white rounded top-[136%] left-[-1.2rem] text-center text-sm`}
                            >
                              Jump to comment
                            </div>
                          </span>
                        ...
                        <div className="flex flex-row gap-4">
                          ...
                          <span className="relative">
                            <FiBookmark
                              onClick={handleSavePostIcon}
                              size={22}
                              onMouseEnter={toggleSecondTooltip}
                              onMouseLeave={toggleSecondTooltip}
                              className={`${
                                isPostSaved ? "fill-black" : ""
                              }  inline cursor-pointer`}
                            />
                            <div
                              className={`${
                                showSaveTooltip ? "visible" : "hidden"
                              } absolute w-28 leading-4 h-9 pt-[7px] bg-gray-900 opacity-90 text-white rounded top-[136%] left-[-1.6rem] text-center text-sm`}
                            >
                ...
                <figure className="mt-10 object-cover flex flex-col items-center justify-center">
                  <img
                    src={`${baseURL}/BlogImages/${blog.mainImage}`}
                    alt="Trulli"
                    onError={(e) => { e.target.onerror = null; e.target.src ='/placeholderImg.png'; }}
                ...
                <div className="bg-[#F9F9F9]  w-full md:mt-16 px-8 py-10 border-b-2 border-gray-300 my-4 flex flex-col justify-start">
                  <div className="flex flex-row justify-between items-center">
                    ...
                    <div className="flex flex-row justify-center items-center gap-x-3">
                      {!token && (
                        <button
                          onClick={() => setAllowedPopUp(true)}
                          className="bg-[#1A8917] text-white px-3 py-2 font-bold rounded-[20px]"
                        >
                          Follow
                        </button>
                      )}
                      {allowedPopUp ? (
                        <FirstDoLoginPopUp setAllowedPopUp={setAllowedPopUp} />
                      ) : (
                        ""
                      )}
                      ...
                      {(
                        !token
                          ...
                          : blog.author?.followers?.length !== 0 &&
                            blog.author?._id !== userId &&
                            blog.author?.followers?.includes(userId)
                      ) ? (
                        <button
                          onClick={() => UserUnFollow(blog.author?._id)}
                          className="bg-[#1A8917] text-white px-3 py-2 font-bold rounded-[20px]"
                        >
                          Following
                        </button>
                        ...
                        blog.author?._id !== userId && (
                          <button
                            onClick={() => UserFollow(blog.author._id)}
                            className="bg-[#1A8917] text-white px-3 py-2 font-bold rounded-[20px]"
                          >
                            Follow
                          </button>
                      ...
                      <AiOutlineMail
                        size={35}
                        color="white"
                        className="p-1 cursor-pointer rounded-full bg-[#1A8917] flex items-center h-full"
                      />
                    </div>
                  </div>
                  <p className="text-2xl font-bold mt-2">
                    Written by {blog.author.fname}
                    {blog.author.lname}
                  </p>
                  <p>{blog.author?.followers.length} Followers</p>
                  <p className=" text-sm mt-3 pb-5 border-b-[1px] border-[rgb(229, 229, 229)] ">
                    {blog.author?.bio}
                  </p>

                  <div className="py-8 mx-auto ">
                    <h2 className="mt-4 mb-8 text-base font-medium">
                      More from {blog.author.fname} {blog.author.lname}
                    </h2>
                    <div className="grid md:grid-cols-2 grid-cols-1 gap-3 mx-auto w-full gap-y-6 border-b-2 border-gray-200">
                      {userRecomenPosts &&
                        userRecomenPosts.map((recom) => (
                          <div key={recom._id} className="flex flex-col">
                            <Link to={`/singleBlog/${recom._id}`}>
                              {" "}
                              <img
                                className="w-[345px] h-[172px] object-cover  cursor-pointer  hover:scale-105 transition-all duration-200"
                                src={`${baseURL}/BlogImages/${recom.mainImage}`}
                            ...
                            <div className="flex flex-row mt-3 mb-3 gap-x-3">
                              <img
                                ...
                                alt="photos"
                              />
                              <p className="pt-2 text-sm text-[#242424]">
                                {blog.author.fname} {blog.author.lname}
                              </p>
                            </div>
                            <Link
                              to={`/singleBlog/${recom._id}`}
                              className="inline"
                            >
                              <h2
                                onClick={() => setApiCalled(false)}
                                className="text-xl text-[#242424] font-bold hover:text-blue-500"
                              >
                                {recom.title}
                              </h2>
                            </Link>
                  ...
                  <span>
                    <Link
                      to={`/writer/${blog.author._id}`}
                      className="mx-3 my-3 md:my-6 px-3 py-2 border-[1px] outline-none border-[rgb(36,36,36)] text-[#242424] text-sm rounded-[20px] inline hover:bg-gray-300"
                    >
                      See all from {blog.author.fname} {blog.author.lname}
                    </Link>
                  </span>
                </div>
                ...
                <section
                  id="comment-section"
                  className="comment-section border-t-2 border-gray-400 mt-2 sm:mt-10"
                >
                  <div className="mb-3">
                    <div className=" text-2xl font-medium ont-bold mt-6 mb-3">
                      {" "}
                      Top comments ({allPostedComment?.length})
                    </div>

                    <div>
                      <form onSubmit={handleCommentOnSubmit}>
                        <textarea
                          className={`border-2 p-3 text-[18px] border-black min-w-[350px] w-full min-h-[100px] transition-all duration-150 rounded resize-none }`}
                        ...
                        <div
                          className={`${
                            btnShow && token ? "flex" : "hidden"
                          } flex-row justify-end items-cenetr py-2 px-2`}
                        >
                          <button
                            disabled={commentData?.commentText?.length === 0}
                            type="submit"
                            className={`px-3 py-2 rounded  mx-2 ${
                              commentData?.commentText?.length == 0
                                ? "bg-[#8992EC] text-[white] cursor-not-allowed"
                  ...
                  {allPostedComment &&
                    allPostedComment.map((comment, idx) => (
                      <div
                        key={idx}
                        className=" flex flex-row justify-start items-start gap-x-1 my-4"
                      >
                        <div className="">
                          <img
                            src={
                              comment?.author?.profileImg
                                ? `${baseURL}/UserImages/${comment?.author?.profileImg}`
                                : imag4
                            }
                            alt="userImg"
                            className=" object-cover  rounded-[50%] h-[48px] w-[45px] border-2"
                          />
                        </div>
                        <div className="w-full border-[1px] rounded border-gray-300">
                          <div className="text-sm flex flex-row justify-start items-center my-3 pl-2 border-b-[1px] border-gray-300">
          ...
          {commentAllowedPopUp && (
            <FirstDoLoginPopUp setAllowedPopUp={setCommentAllowedPopUp} />
          )}
          {postSavedAllowedPopUp && (
            <FirstDoLoginPopUp setAllowedPopUp={setPostSavedAllowedPopUp} />
          )}
        </div>
      )}
