...
const WriterProfilePage = () => {
  ...
  const fetchUserData = async () => {
    try {
      const res = await axios.get(
        `${baseURL}/user/getRandomUserInfo/${userId}`
      );
      if (res.status == 200) {
      ...
      if (
        ...
        error.response.data.error == "Invalid userId ID"
      ) {
  ...
  const getAllPostToSpecificUser = async () => {
    ...
    try {
      const res = await axios.get(
        `${baseURL}/post/getAllPostToSpecificUser/${userId}`
      );
      if (res.status == 200) {
        console.log(res.data.data);
  ...
  return (
    <div className="bg-[#F5F5F5]">
      <div className="w-full  min-h-[475px] m-auto  bg-[#F5F5F5] relative">
        <div
          className={`relative h-36 w-full flex items-end justify-center `}
          style={{
            backgroundColor: `${
              userInfo.brandColor ? userInfo.brandColor : "black"
            }`,
          }}
        >
          <div className="absolute max-w-[1000px] w-full bg-white rounded-[6px] min-h-[22rem] top-[50%] custome-shadow p-2">
            <div className="text-right mr-8 mt-3">
              {!token && (
                <button
                  ...
                  className="bg-[#3bdf6a] mr-3 px-4 py-2 rounded outline-none font-bold text-white"
                >
                  Follow
                </button>
              )}
              ...
              {!token ? (
                ...
                userInfo._id !== selfUserId &&
                userInfo.followers?.includes(selfUserId) ? (
                <button
                  onClick={() => UserUnFollow(userInfo._id)}
                  className="bg-gray-200 mr-3 px-4 py-2 rounded outline-none font-bold text-black"
                >
                  Following
                </button>
              ) : (
                userInfo._id !== selfUserId && (
                  <button
                    onClick={() => UserFollow(userInfo._id)}
                    className="bg-[#3B49DF] mr-3 px-4 py-2 rounded outline-none font-bold text-white"
                  >
                    Follow
                  </button>
                )
              ...
              <div className=" inline relative">
                <BsThreeDots
                  ...
                  className={`inline  rounded py-2 px-2 cursor-pointer ${
                    reportBoxopen ? "bg-gray-200" : ""
                  }`}
                />
                <div
                  className={`absolute  text-left  top-[180%] right-[5%] h-20 md:w-[220px] w-[180px] border-2 z-30 text-black bg-white p-2 py-3 ${
                    reportBoxopen ? "block" : "hidden"
                  } custome-shadow rounded`}
                >
                  <p className="cursor-pointer hover:text-blue-500">
                    Report abuse
                  </p>
                  <p className="mt-1 cursor-pointer hover:text-blue-500">
                    Blocked user
                  </p>
                </div>
              </div>
            ...
            <div className="Information-box  mt-12">
              ...
              <p className="text-center  max-w-2xl w-full m-auto mt-4">
                👨‍💻{userInfo.bio}.
              </p>
              <div className="flex flex-row flex-wrap justify-center gap-x-12 gap-y-3 items-center mt-2">
                <div className="mr-5">
                  <FaLocationDot
                    size={20}
                    className={` mr-2 ${
                      userInfo.location ? "inline" : "hidden"
                    }`}
                  />
                  <span>{userInfo?.location}</span>
                </div>
                <div className="mr-5">
                  <FaBirthdayCake size={20} className="inline mr-2" />
                  <span>Joined {formatDate(new Date(userInfo.createdAt))}</span>
                </div>
                <div className="mr-5">
                  <GrMail size={20} className="inline mr-2" />
                  <a
                ...
                <div className="mr-5">
                  <AiFillGithub size={30} className="inline mr-2" />
            ...
            <div className="border-t-[1px] border-gray-300 mt-4 pt-6 flex flex-col justify-center items-center">
              <p>Work</p>
              {userInfo.work ? (
                <p className="max-w-xl  w-full text-center m-auto mb-1">
                  {userInfo?.work}
                </p>
              ) : (
        ...
        <div className="absolute w-36 h-36 bg-black rounded-full top-[4%] lg:left-[44%] md:left-[35%] sm:left-[15%] left-[10%] overflow-hidden border-2 ">
          {userInfo?.profileImg ? (
            <img
              src={
                userInfo?.profileImg &&
                `${baseURL}/UserImages/${userInfo?.profileImg}`
              }
              alt=""
            ...
            <div
              className="text-6xl h-full text-center flex justify-center items-center"
      ...
      <div className="max-w-[1000px] w-full  min-h-[600px] m-auto flex flex-row justify-between gap-x-4 ">
        <div className="left hidden lg:block max-w-[320px] w-full h-48 mt-3 ">
          <div className="custome-shadow mb-4 rounded bg-white ">
            <h1 className="text-xl font-medium border-b-[1px] border-gray-200 py-3 px-5">
              Toatal Publish Posts
            </h1>
            <p className="text-gray-600 text-[xl] px-5 py-2">
              {AllUserPost?.length}
            </p>
          </div>
          ...
          <div className="custome-shadow mb-4 rounded bg-white ">
            ...
            <p className="text-gray-600 text-[xl] px-5 py-2">
              {userInfo.skillLanguage ? userInfo?.skillLanguage : "Not given"}
            </p>
          </div>
          ...
          <div className="custome-shadow mb-4 rounded bg-white ">
            ...
            <p className="text-gray-600 text-[xl] px-5 py-2">
              {userInfo.available ? userInfo?.available : "Not given"}
            </p>
          </div>
        </div>
        <div className="right w-full p-3">
          {/* {AllUserPost.length == 0 && (
            <div className="text-center text-xl font-bold border-2 flex flex-row justify-center items-center border-gray-300  m-1 h-32">
              {" "}
...
<div className='right w-full p-3'>
  {AllUserPost.length==0 && postLoading ? ( // Initial loading state
    <div className="flex justify-center items-center py-10 w-full border-gray-400 border-2 rounded-md h-56">
      <div className="inline-block animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-blue-500"></div>
    </div>
  ) : AllUserPost.length === 0 ? ( // No posts state
    <div className='text-center text-xl font-bold border-2 flex flex-row justify-center items-center border-gray-300 m-1 h-32'>
      No Posts Available
    </div>
  ) : (
    AllUserPost.map((post) => (
        <article
              key={post._id}
              className="flex bg-white  w-full lg:mx-0 lg:max-w-2xl flex-col items-start justify-between p-3 rounded-[6px] shadow-sm border border-gray-300 my-4 sm:my-1"
            >
              <div className="group relative">
                <div className="relative mt-5 flex items-center gap-x-4">
                  {/* <img src={post.author?.profileImg?`${baseURL}/UserImages/${post.author?.profileImg}`:`/assets/userLogo.jpg`} alt="" className="h-10 w-10 rounded-full bg-gray-50" /> */}
                  <img
                    src={
                      post.author?.profileImg
                        ? `${baseURL}/UserImages/${post.author.profileImg}`
                        : "/userLogo1.jpg"
                    }
                    ...
                    onError={(e) => {
                      ...
                      e.target.src = "/assets/userLogo.jpg";
                    }}
                  />
